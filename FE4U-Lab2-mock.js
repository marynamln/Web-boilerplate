...
function mergeUsers(randomUserMock, additionalUsers) {  
  const combinedUsers = [...randomUserMock, ...additionalUsers];

  const uniqueUsersMap = new Map(combinedUsers.map(user => [user.full_name, user]));
...
function startsWithUppercase(str) {
  return typeof str === 'string' && str.length > 0 && str.charAt(0) === str.charAt(0).toUpperCase();
}
...
function validateUser(user) {
  ...
  stringFields.forEach(field => {
    if (!startsWithUppercase(user[field])) {
      errors.push(`${field} має бути рядком і починатися з великої літери.`);
    }
  });
  ...
  if (!validatePhoneByCountry(user.phone, user.country)) {
    errors.push('Phone має відповідати формату країни.');
  }
...
function findUsers(users, searchParam, searchValue, operator = '==') {
  return users.filter(user => {
      if (typeof user[searchParam] === 'number') {
          switch (operator) {
              case '>':
                  return user[searchParam] > searchValue;
              case '<':
                  return user[searchParam] < searchValue;
              case '>=':
                  return user[searchParam] >= searchValue;
      ...
      } else if (typeof user[searchParam] === 'string') {
        return user[searchParam].toLowerCase().includes(searchValue.toLowerCase());
      }
      return false;
  });
}
...
function getMatchingPercentage(users, searchParam, searchValue, operator = '==') {
  const matchingUsers = findUsers(users, searchParam, searchValue, operator);
